...
n = readInt()
cntList = [dict() for _ in range(20)]
# print(cntList)
for a0 in range(n):
    s = readStr()
    length = len(s)
    for i in range(length):
        if s[i] in cntList[i]:
            cntList[i][s[i]].append(a0)
            # print("loop1")
            # print(cntList)
        else:
            cntList[i].update({s[i]: [a0]})
            # print("loop2")
            # print(cntList)
...
for _ in range(q):
    ...
    p = int(p)-1
    if c in cntList[p]:
        allSWithCAtP = cntList[p][c]
        # print(allSWithCAtP)
        L = bisect.bisect_left(allSWithCAtP, l)
        R = bisect.bisect_right(allSWithCAtP, r)
        # print(L,R)
        print(R-L)
    else:
        print(0)
